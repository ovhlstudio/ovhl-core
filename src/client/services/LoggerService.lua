...
LoggerService.__manifest = {
    ...
    priority = 100,
    domain = "core",
    description = "Client-side structured logging dengan EMOJI!"
}
...
function LoggerService:Init() self:Info("🔧 Client LoggerService initialized") return true end
function LoggerService:Start() self:Debug("🚀 Client LoggerService started") end
...
function LoggerService:_formatMessage(level, message, context)
    ...
    if self._config.EnableTimestamps then table.insert(parts, "[" .. os.date("%H:%M:%S") .. "]") end
